...
instructions, with one instruction per line. However, AbstractVM’s assembly languagehas
a limited type system, which is a major difference from other real world assembly languages.
...
• min: Finds the minimal value in stack. If the number of values on the stack is strictly
inferior to 2, the program execution must stop with an error.

• max: Finds the maximal value in stack. If the number of values on the stack is strictly
inferior to 2, the program execution must stop with an error.

• avg: Finds the average value in stack. If the number of values on the stack is strictly
inferior to 2, the program execution must stop with an error.
